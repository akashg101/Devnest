      ...
      <div className="dark:bg-darkmode grid lg:grid-cols-2 grid-cols-1 md:gap-20 gap-4 items-center border-t border-solid border-AliceBlue dark:border-dark_border lg:px-0 px-7">
        ...
        <div className="xl:pb-0 pb-5">
          ...
          <p className="text-lg font-normal text-SlateBlue dark:text-darktext max-w-550 md:pb-16 pb-6">
            I follow a proven methodology to deliver high-quality WordPress, PHP, Laravel, and Shopify solutions. Each project is handled with precision, from discovery to deployment and beyond.
          </p>
          <div className="grid md:grid-cols-2 grid-cols-1 gap-y-8">
            <div className="flex items-center gap-5">
              <div className="bg-cyan-500 dark:bg-cyan-600 w-8 h-8 flex items-center justify-center rounded-full">
                <span className="text-white text-14 font-bold">1</span>
              </div>
              <div>
                <p className="font-semibold text-lg text-secondary dark:text-white">Discovery & Requirements</p>
                <p className="text-sm text-SlateBlue dark:text-darktext mt-1">Understand your goals, project scope, timeline, and technical requirements for optimal solution design.</p>
              </div>
            </div>
            <div className="flex items-center gap-5">
              <div className="bg-cyan-500 dark:bg-cyan-600 w-8 h-8 flex items-center justify-center rounded-full">
                <span className="text-white text-14 font-bold">2</span>
              </div>
              <div>
                <p className="font-semibold text-lg text-secondary dark:text-white">Planning & Architecture</p>
                <p className="text-sm text-SlateBlue dark:text-darktext mt-1">Design the technical architecture, database schema, and create detailed project roadmap for implementation.</p>
              </div>
            </div>
            <div className="flex items-center gap-5">
              <div className="bg-cyan-500 dark:bg-cyan-600 w-8 h-8 flex items-center justify-center rounded-full">
                <span className="text-white text-14 font-bold">3</span>
              </div>
              <div>
                <p className="font-semibold text-lg text-secondary dark:text-white">Development & Integration</p>
                <p className="text-sm text-SlateBlue dark:text-darktext mt-1">Build clean, scalable code with best practices. Integrate APIs, plugins, and custom features seamlessly.</p>
              </div>
            </div>
            <div className="flex items-center gap-5">
              <div className="bg-cyan-500 dark:bg-cyan-600 w-8 h-8 flex items-center justify-center rounded-full">
                <span className="text-white text-14 font-bold">4</span>
              </div>
              <div>
                <p className="font-semibold text-lg text-secondary dark:text-white">Testing & Deployment</p>
                <p className="text-sm text-SlateBlue dark:text-darktext mt-1">Rigorous QA testing, performance optimization, security checks, and smooth production deployment.</p>
              </div>
            </div>
          </div>
        </div>
